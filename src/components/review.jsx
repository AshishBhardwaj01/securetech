...
const FrameComponent1 = ({ className = "" }) => {
  ...
  const handlePrev = () => {
    setCurrentIndex((prevIndex) => (prevIndex - visibleReviews + reviews.length) % reviews.length);
  };

  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex + visibleReviews) % reviews.length);
  };
  ...
  return (
    <div className={`flex flex-col gap-5 ${className}`}>
      ...
      <div className={`flex items-center justify-between gap-10 lg:gap-1 p-10 ${className}`}>
        <button 
        ...
        <div className="flex flex-row lg:flex-col gap-10 lg:gap-2 items-center">
          {reviews.slice(currentIndex, currentIndex + visibleReviews).map(review => (
            <div key={review.id} className="shadow-md rounded-lg p-6 bg-slate-200 w-80 h-96 lg:w-44 lg:h-96 relative">
              ...
              <div className="mt-16 flex flex-col items-center space-y-2">
                <h3 className="text-md lg:text-mini font-poppins font-semibold">{review.name}</h3>
                <p className="text-sm lg:text-xs text-black font-poppins text-center">{review.feedback}</p>
                <div className="flex">
