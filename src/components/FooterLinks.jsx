...
const FooterLinks = ({ className = "" }) => {
  return (
    <footer className={`max-w-full font-poppins bg-slate-900 flex flex-col text-white ${className}`}>
      <div className="flex flex-row items-center justify-center p-5 gap-8 sm:gap-10 md:gap-12 lg:gap-14">
        <SocialLink href="https://www.linkedin.com/company/securetechav-designs/" src="/linkdin.svg" alt="LinkedIn" />
        <SocialLink href="https://instagram.com/secuetechav?utm_source=qr" src="/instagram.svg" alt="Instagram" />
...
const SocialLink = ({ href, src, alt }) => (
  <a href={href} target="_blank" rel="noopener noreferrer">
    <img className="cursor-pointer h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8" alt={alt} src={src} />
  </a>
);
...
const NavLink = ({ href, text }) => (
  <a href={href} className="tracking-tight no-underline leading-snug font-light text-inherit hover:underline cursor-pointer">
    {text}
  </a>
);
...
const Copyright = () => (
  <div className="w-full lg:text-xs flex flex-col items-center lg:items-start justify-center p-4 lg:p-1 text-xs sm:text-sm text-white font-poppins">
    ...
    <div className="flex flex-wrap justify-center items-center gap-2 lg:gap-1">
      ...
      <span className="border-l border-white "> Cookie Policy & Settings |</span>
